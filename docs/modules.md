...
| Header / source | Responsibility |
| --- | --- |
| `app.h/.cpp` | Window, toolbar, theme, About overlay, Generate/Build/Run, autosave, **live validation debounce**, Call→FunctionDef helper |
| `canvas_view.h/.cpp` | Pan/zoom, select, **resize handle**, per-block fill/text colors, wire **reconnect**, optional **orthogonal** wires, place, clipboard, tidy, minimap, FunctionDef collapse, validation outlines, Call arity subtitle |
| `palette.h/.cpp` | Collapsible block groups + filter + drag-and-drop place |
| `toolbar.h/.cpp` | File / build / layout / **Ortho** / Theme / About / Help actions |
| `property_panel.h/.cpp` | Edit properties (blinking **caret**, Left/Right/Home/End); style fields (`width`/`height`, color presets + Custom hex); Document fields when nothing selected |
| `log_pane.h/.cpp` | Compiler (click-to-jump / live issues); Program Output + stdin with caret; Help/About overlay text + clickable http(s) links |
| `ui_theme.h/.cpp` | Dark/Light SFML color palettes (`GetUiTheme`) |
| `ui_theme_id.h/.cpp` | Theme id + string conversion (also in `cgen_core`) |
| `node_style.h/.cpp` | Per-block size/color helpers: presets, hex parse, property keys (`width`, `fillColor`, …) — also in `cgen_core` |
...
| `block_placement.h/.cpp` | Non-overlapping placement + `ComputeBlockNodeWidth` / Height (custom size) + title-band / port layout — also in `cgen_core` |
...
`tests/test_*.cpp` exercise `cgen_core` (document, types, validator, severity map, codegen including per-block comments, serializer, autosave, layout, align, placement, **node style** / hex presets, history, theme ids, tooltip text). They do not drive the SFML UI.
