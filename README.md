...
[`examples/dungeon_log.cgen`](examples/dungeon_log.cgen) is a CLI adventure that showcases nearly every block family (control, arithmetic, strings, files, time, structs, functions). Regenerated by [`scripts/generate_dungeon_log.py`](scripts/generate_dungeon_log.py).
...
- **Malloc** / **Free** are omitted on purpose: `void*` cannot be stored through typed Assign/Init in this IR (use **Array Decl** for buffers instead)
